#ifndef CATTopCorner_H
#define CATTopCorner_H
...
*</ul>
*/
class ExportedByParallel CATTopCorner : public CATTopTangentCurveOperator
{
  CATCGMVirtualDeclareClass(CATTopCorner);
public :
	
	virtual              ~CATTopCorner();
/**
  * @nodoc
	* Constructor. Cannot be called. Use the <tt>CreateTopOpCorner</tt> global function
	* to create a CATTopCorner operator.
	*/
	CATTopCorner (CATGeoFactory *, int iNbWires);
	/** @nodoc */
	CATTopCorner (CATGeoFactory *, CATTopData * iData, int iNbWires);
...
/**
	...
	* tritangent corner.
	*/
	virtual void GetCurOrients (CATOrientation &ioOr1, CATOrientation & ioOr2, CATOrientation &ioOr3) const = 0;
 ...
 */
	virtual void SetOffsetOrientation(int iOrientCrv1, int iOrientCrv2, int iOrientCrv3=0) =0;
 ...
 * the two sides parallels are computed. In case of tritangent corner only.
	*/
	virtual void GetOffsetOrientation(int &ioOrientCrv1, 
		int &ioOrientCrv2, int &ioOrientCrv3) const=0;
...
/**
	...
	*/
    ...
    /**
    * @nodoc
    * Defines whether the input wires must be assembled in the resulting body.
    ...
    */
    ...
    /**
    * @nodoc
    * Returns whether the input wires must be assembled in the resulting body.
    * In this case, the iterator cannot be used.
    * @param iTrimmingMode
    * The assembly mode.
    * <br><b>Legal values</b>:
    ...
    * In this last case, <tt>CATCircleMode</tt> must changed to <tt>Direct</tt>.
    */	
  virtual CATLONG32 GetSupportTrimmingMode     (CATLONG32 & iTrimmingMode)=0;
  ...
  * In this last case, <tt>CATCircleMode</tt> must changed to <tt>Direct</tt>.
  */
  virtual CATLONG32 SetSupportTrimmingSideAndMode(const CATLONG32 iTrimmingSide,const CATLONG32 iTrimmingMode)=0;
  ...
  * <tt>0</tt> if the assembly is compatible with the relimitation mode, <tt>1</tt> otherwise. 
  */
  virtual CATLONG32 GetSupportTrimmingSideAndMode(CATLONG32 & oTrimmingSide, CATLONG32 & oTrimmingMode)=0;

/** @nodoc */
  virtual CATLONG32 SetSupportTrim  (const CATLONG32 iTrimmingSide, const CATLONG32 iTrimmingMode)=0;

/** @nodoc */
  virtual CATLONG32 GetSupportTrim  (CATLONG32 iTrimmingSide, CATLONG32 & oTrimmingMode)=0;
  ...
  /**
	...
	*/
	virtual CATLONG32 SetCircleMode(CATCircleMode iCircleMode, 
		double iStart=0.0, double iEnd=0.0)=0;
...
/**
	...
	* The value is not touched.
	*/
	virtual void GetCircleMode(CATCircleMode & ioCircleMode, 
		double &ioStart,double &ioEnd)=0;
/** @nodoc */
...
/**
* Creates a CATTopCorner operator that computes a rounded corner between two wires lying
* on a shell domain.
...
*/
ExportedByParallel
CATTopCorner * CATCreateTopCorner (CATGeoFactory     * iFactory,
				   CATTopData * iTopData,
				   CATBody      * iCurve1,
				   CATBody      * iCurve2,
				   CATBody       * iSupport,
