#ifndef CATFrFTopologicalSweep_H
#define CATFrFTopologicalSweep_H
...
/**
 * Defines a list of computed curves.
 */
 ...
 *</ul>
 *<br>
 * The CATFrFTopologicalSweep operator follows the global frame of the topological operators 
 * and satisfies the smart mechanism:
 * the input bodies are not modified. A new resulting body is created,
 * possibly sharing data with the input bodies.
 *<ul>
 * <li>A CATFrFTopologicalSweep operator is created with the <tt>CATCreateFrFTopologicalSweep</tt> global method:
 ...
 *</ul>
 */
class ExportedByFrFTopologicalOpe CATFrFTopologicalSweep  : public CATTopOperator
{
  CATCGMVirtualDeclareClass(CATFrFTopologicalSweep);
  public :

/**
 * @nodoc
 * Constructor. Do not use. Use the <tt>CATCreateFrFTopologicalSweep</tt> global functions to create a
 * CATFrFTopologicalSweep operator.
 */
                   CATFrFTopologicalSweep                 (CATGeoFactory               * iFactory                  ,
                                                           CATTopData                  * iTopData                  ,
                                                           CATExtTopOperator           * iExtTopOperator           ) ;

  // Destructor
  virtual         ~CATFrFTopologicalSweep                 ();
...
/**
 * @nodoc
 * Returns the access to the internal operator.
 * Internal use.
 */
...
/**
 * @nodoc
 * Returns the access to the new internal operator.
 * Internal use.
 * Note for application programmers:
 * not necessary anymore; all boolean options may now be set through
 * standard interface or through internal Sweep adapter
 */
  virtual CATSweep2* GetSweep2Operator                    ()                                                            = 0 ;


/**
 * @nodoc
 * Returns the access to the internal sweep checker.
 * Internal use.
 */
...
/**
 * @nodoc
 * Access to some internal sweep data.
 * Internal use.
 */
  virtual void     SweepDataAccess                        (CATFrFTopologicalSweepDataAction
                                                                                         iAction                   ,
                                                           void                       *& ioInternalData            )    = 0 ;
 ...
 */
  ...
  //-----------------------------------------------------------------------
  //--------------- Methods for Debug purpose only -------------------------
  //-----------------------------------------------------------------------
/**
 * @nodoc
 * Initialization of one debug mode. Methods for Debug purpose only
 */
  virtual void     SetDebugMode                           (CATFrFTopologicalSweepDebug_Mode
                                                                                         iMode                     )    = 0;
 ...
 */
  virtual void     SetBehaviourMode                       (CATFrFTopologicalSweepBehaviour_Mode
                                                                                         iMode                     )    = 0;
/**
 ...
 * @nodoc
 * Sample:
 * CATFrFTopologicalSweep* p ;
 *  CATFrFTopologicalSweepBehaviour_Mode SaveMode ;
 *   SaveMode = p.GetBehaviourMode(CATFrFTopologicalSweepBehaviour_GetFrameType)
 *   p.SetBehaviourMode(CATFrFTopologicalSweepBehaviour_FinalFrameType) ;
 *   p.GetFrame(lambda)
 *   // restore previous mode
 *   p.SetBehaviourMode(SaveMode)
 *  or
 *   // restore default mode
 *   p.SetBehaviourMode(CATFrFTopologicalSweepBehaviour_ResetFrameType) ;
 */
  virtual CATFrFTopologicalSweepBehaviour_Mode 
                   GetBehaviourMode                      (CATFrFTopologicalSweepBehaviour_Mode
                                                                                         iQuery                    )    = 0;
...
/**
 * @nodoc
 * Advanced local C0 management
 * (incompatible with untwist and pseudo-untwist options)
 */
 ...
 * @nodoc
 */
  virtual void     SetSectionSolverEngine                 (CATSectionSolverEngine      * iSolverEngine             )    = 0 ;
  //
/**
 * @nodoc
 * Gets deviation from a specification (guide or profile)
 * to the computed sweep
 * 1<=iNumSpec<=number of guides   for computation on one guide
 * 1<=iNumSpec<=number of profiles for computation on one profile
 * iNumSpec is meaningless in other cases
 * (max on guides, max on profiles, max on guides and profiles)
 */
  virtual void     GetDeviation                           (CATFrFTopologicalSweepDeviation_Type
                                                                                         iComputationMode          ,
                                                           int                           iNumSpec                  ,
                                                           CATTransitionCheckingCommand  iCmd                      ,
                                                           CATTransitionDeviationResult& oDeviationResult          ,
                                                           CATTransitionPositionResult & oPositionResult           )    = 0 ;
/**
...
/**
 * @nodoc
 * Force topological report level (otherwise report level is upgraded throughout software configuration)
 * iLevel = -1 : best up to date level (subject to change)
 * iLevel =  0 : old (before nov 99)
 * iLevel =  1 : new (since nov 99)
 * iLevel =  2 : newer (since May 2000) = 1 + debug
 * ... up to iLevel = 13  (since October 2010)
 */
  virtual void     ForceReportLevel                       (int                           iLevel                    =-1) = 0 ;

/**
 * @nodoc
 * Defines the journal versionning.
 * 1: first version (before nov 99); 0: current version.
 */
  virtual void     SetOldJournal                          (int                           onoff                     )    = 0;
...
/**
 * @nodoc
 * Access to moving frame (pointer remains to this object: no object duplication)
 */
 ...
 *   The interface
 */
 virtual CATIPGMFrFMovingFrame* GetIMovingFrame           ()                                                         = 0 ;
...
/**
 * @nodoc
 * Defines approximation tolerance.
 * Incompatible with SetCleanGuidesOption or SetCleanProfilesOption that lead
 * to a different value automaticcaly computed.
 */
 virtual void     SetApproximationTolerance               (double                          iApproximationTolerance )       = 0 ;
 ...
 * @nodoc
 * Internal use.
 * Definition of a function to call after each step, during run
 * and of data to pass to this function.
 */
 virtual void     SetStepByStepReCall                     (void(*ReCall)(void* ioData)                             = NULL,
                                                           void                          * ioDataForCall           = NULL) = 0;
