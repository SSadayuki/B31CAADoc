...
//--------------------------------------------------------------------------------------------------
  ...
  cout << "   AddExtension for " << strHistoricExtNovel.ConvertToChar() << " OK" << endl;
  ...
  if (FAILED(rc)) return 1;
  cout << "   AddExtension for " << strBiographicalExtNovel.ConvertToChar() << " OK" << endl;
  CATFmFeatureFacade FeatFacadeOnBiogNovelExtn(MyCredential, attrValue);

  // 5-3- Instantiate the Childrens Novel Extension
  CATString strChildrensExtNovel("`CAAOsmChildrensNovel`@`CAAOsmExt2Catalog.CATfct`");
  rc = FeatFacadeOnNovelInstBaseFeat.AddExtension(strChildrensExtNovel, attrValue);
  if (FAILED(rc)) return 1;
  cout << "   AddExtension for " << strChildrensExtNovel.ConvertToChar() << " OK" << endl;
  ...
  if (strcmp(IdExt.ConvertToChar(), strChildrensExtNovel.ConvertToChar())) return 1;

  cout << "   OK: Its identifier is " << IdExt.ConvertToChar() << endl << flush;
  ...
  //-------------------------------------------------------------------------
    // 7-1- Retrieve the BiographicalNovel extension from the Novel feature (base)
  CATString iExtIdForBiogNovel("`CAAOsmBiographicalNovel`@`CAAOsmExt1Catalog.CATfct`");
  ...
  if (strcmp(IdExt.ConvertToChar(), iExtIdForBiogNovel.ConvertToChar())) return 1;
  cout << "   OK: Its identifier is " << IdExt.ConvertToChar() << endl << flush;
  ...
  cout << "   Epoch = " << EpochName.ConvertToChar() << endl << flush;
  ...
  cout << "   Domain = " << DomainName.ConvertToChar() << endl << flush;
