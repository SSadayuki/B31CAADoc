/**
 * @quickreview VD6 19:09:20 IR-516722 Modified GetDocInSession(), CastToCharPtr() was depricated from CXR25, now using ConvertToChar API.
...
CATDocument * CAASchAppBaseServices::GetDocInSession (const char *iDocType)
{
   ...
   if (NULL == pDoc)
   {
     ...
     if (NULL != LDoc)
     {
        ...
        {
           pDocMem = (*LDoc)[iDoc];         
           if ( NULL != pDocMem && pDocMem->IsAKindOf(SDocType.ConvertToChar())) // IR-516722 Fix : CastToCharPtr() was depricated from CXR25, hence now using ConvertToChar API.
           {
              pDoc = pDocMem;
              break;
           }
