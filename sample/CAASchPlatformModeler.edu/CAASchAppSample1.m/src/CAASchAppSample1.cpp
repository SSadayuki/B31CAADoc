/**
* @quickreview VD6 19:09:20 IR-516722 Modified GetDraftingObjects(), CastToCharPtr() was depricated from CXR25, now using ConvertToChar API.
* @fullreview
**/
//=============================================================================
  ...
  CATTry 
    ...
    for (int iSheet = 1; (iSheet <= SizeOfLSheet) && (NULL_var == _spDetailSpec); iSheet++)
    {
       ...
       cout << "Member sheet: " << iSheet << "is "
            << (spShAlias->GetAlias()).ConvertToChar() << endl; // IR-516722 Fix : CastToCharPtr() was depricated from CXR25, hence now using ConvertToChar API.
