 ...
 * @error MISC Y
 * @error MISC Y
* @quickreview F9G GQT 19:01:22 Changes to validate the Correction for IR-659681:COTT in CATIElbSupport::ListSegments
*/
/**
...
{ 
  ...
  for( int iCase = 1; iCase <= nbOfCases; iCase++ )
  {
    ...
    for ( int i=1; i<=NumberOfSupp ; i++)
    {
      ...
      CATListValCATBaseUnknown_var *pListOfSegments3 = NULL;
      ...
      if (SUCCEEDED(rc) && (NULL!= pSupport))
      {
        ...
        {               
          ...
          rc = pSupport->ListSegments(&pListOfSegments3, 1);
          if (SUCCEEDED(rc) && ((1 == pListOfSegments3->Size())))
            cout << "ListSegments successful. " << endl << flush;
          else
          {
            cout << "ERROR : ListSegments failed. " << endl << flush;
