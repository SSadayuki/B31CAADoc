...
// Mission         : Explains how to create a circle sweep with 
//                   one guide - a tangency surface - a radius 
//           
// Type            : Batch program
// Inputs          : None
...
//                   2- Create 2 splines (one is the guide - the other is to be extruded)
//                   3- Create the tangency surface by extruding one of the spline
//                   4- Create the sweep circle and retrieve one of the result body
//                   5- Clean the no longer required objects 
//                   6- Write the model and close the container  
//
// How to run it   : CAAAdtCircleSweepTangSurRadius    // to run whithout storing the NCGM file
//                   CAAAdtCircleSweepTangSurRadius file.NCGM // to run and store the result 
//                   in file.NCGM     
//==================================================================
...
#include "CATMathPoint.h"                // Mathematical point creation
...
#include "CATConstantLaw.h"              // Constant law
#include "CATCrvLimits.h"                // Curve limits
#include "CATCurve.h"       


// NewTopologicalObjects
#include "CATTopMultiResult.h"           // To manage topological operators with
                                         // several body result
...
#include "CreateTopSweepCircleWireSkin.h"  // To create the sweep operator 
         ...
         char** iArgv)   // Path to the *.NCGM document generated by this program
{
    ...
    CATBody * pPBody = pPrismOp->GetResult();
    if (NULL==pPBody)
    {
        cout << "Problem when creating the prism  " << endl;
        ::CATCloseCGMContainer(piGeomFactory);
        return (1);
    }
    ...
    int iBody = 1;
    for ( iBody = 1 ; iBody <= nbBodies ; iBody ++)
    {
        pWireSkinCircleOpe->NextResult();
        if (iBody == 2)
        {
            CATBody * pCurBody = pWireSkinCircleOpe->GetResult((CATCGMJournalList*)NULL );
        }
    }  

    delete pWireSkinCircleOpe;
    pWireSkinCircleOpe=NULL;
