         ...
         char** iArgv)   
{
    ...
    { 
      ...
      if (NULL!=ShellOrientationOpe)
      {
        ...
        CATTry
        {
          result = ShellOrientationOpe->Run();
        }
        CATCatch(CATError,error) //WMN20181106_POLYCRASH stack 102
        {
          if (error) Flush(error); error=NULL;
          result = 1;
        }
        CATEndTry;
        ...
        delete ShellOrientationOpe; ShellOrientationOpe=NULL;
            ...
            int i = 1;
				for (  i=1; i<=newListOfFaces.Size(); i++)
				{
...
#else
        ofstream filetowrite(pFileName1,ios::out) ;
#endif
